...
LoadVariables_compound <- function(dataset){
  ...
  dataset <- dataset %>% mutate(start_time = as.numeric(gsub(":",".",substring(pmax(E_START_TIME,ADD_TIME_EXCH,na.rm=TRUE),1,5))))
  
  #convert E_START_TIME to numeric values
  dataset <- dataset %>% mutate(end_time = as.numeric(gsub(":",".",substring(pmin(E_END_TIME,FIN_TIME,na.rm=TRUE),1,5))))
  ...
  #-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  #return
  dataset <- dataset %>% mutate(return_pct = ifelse(is.na(LAST_CHG),-sign(E_QTY_DONE)*log(PREV_WAVE_MID_AT_END/MARK),-sign(E_QTY_DONE)*log(LAST_CHG/MARK)),
  ...
  #change the name of EXEC_CT and MARK
  #-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------

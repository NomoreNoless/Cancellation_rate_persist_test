...
Load_compound_variables <- function(dataset,FirmName,time_range){
  ...
  for(year_month in time_range){
    ...
    timebin_cancelrate <- timebin_cancelrate(msgs10_separate)
    
    dataset_temp <- inner_join(dataset_temp,timebin_cancelrate,by=c("FirmName","TethysClientID","StrategyID","CompoundID","Ticker"))
